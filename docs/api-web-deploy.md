...
- The Cloud Run service uses internal ingress and runs under a dedicated least-privilege runtime service account (`evi-cloud-run-runtime@...`, created by `scripts/setup_gcp.sh`). It does not use the default compute SA. The runtime service account has bucket-scoped GCS object-read and nothing else: no BigQuery and no project Editor. The request path only reads model/enrichment artifacts. Deploy paths attach the service account via `--service-account` and refuse to deploy if it is missing, so the service can't silently fall back to the broad default identity.
