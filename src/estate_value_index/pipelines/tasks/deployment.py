...
def _describe_service(service_name: str, region: str, field: str) -> str | None:
...
def _get_current_revision(service_name: str, region: str) -> str | None:
    """Get the currently serving revision."""
    return _describe_service(service_name, region, "status.latestReadyRevisionName")


def _get_service_url(service_name: str, region: str) -> str | None:
    """Get the service URL."""
    return _describe_service(service_name, region, "status.url")
...
def deploy_to_cloud_run_task(
    ...
    region = region or os.getenv("CLOUD_RUN_REGION", "europe-north1")
    project_id = os.getenv("GCP_PROJECT_ID", "estate-value-index")
    ...
    current_revision = _get_current_revision(service_name, region)
    ...
    service_url = _get_service_url(service_name, region)
    new_revision = _get_current_revision(service_name, region)
...
def rollback_deployment_task(
    ...
    active_revision = _get_current_revision(service_name, region)
...
def monitor_pipeline_health_task() -> dict:
    ...
    try:
        ...
        project_id = os.getenv("BIGQUERY_PROJECT_ID", "estate-value-index")
