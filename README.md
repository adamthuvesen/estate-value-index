...
```bash
pytest tests/ml/test_temporal_leakage.py
pytest tests/api/test_model_integrity.py tests/test_api_server.py
pytest tests/utils/test_bigquery_safety.py tests/ml/test_where_clause_call_sites.py
```
...
```bash
python train_model.py --use-features --tune
```
...
python -m src.estate_value_index.pipelines.core.complete_pipeline --quick

# Production-oriented run
python -m src.estate_value_index.pipelines.core.complete_pipeline --production

# Retrain on existing data and deploy
python -m src.estate_value_index.pipelines.core.complete_pipeline --retrain --deploy
```

See all flags:

```bash
python -m src.estate_value_index.pipelines.core.complete_pipeline --help
```

### Ingest listings

```bash
python -m src.estate_value_index.cli.crawl_booli --max-pages 10
```
...
```bash
python -m src.estate_value_index.utils.settings
```
