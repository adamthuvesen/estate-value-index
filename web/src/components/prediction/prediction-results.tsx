...
export function PredictionResults({
  ...
  return (
    <aside className="space-y-4 lg:col-span-1">
      <div className="ledger-card p-6 lg:sticky lg:top-20">
        <h3 className="text-[15px] font-semibold text-ledger-text">Estimated value</h3>

        {prediction ? (
          <div className="mt-4 space-y-5">
            <div>
              <p className="num text-[34px] font-semibold leading-none tracking-tight text-ledger-text">
                {formatDisplayPrice(currencyFormatter, prediction.rounded_predicted_price)}
              </p>
              <p className="mt-2 text-[13px] text-ledger-muted">Estimated range around rounded model estimate</p>
              <p className="num mt-1 text-[13px] font-medium text-ledger-text">
            ...
            <dl className="space-y-0 text-[13px]">
              ...
              <Row label="Confidence">
                <span className="text-ledger-muted">{prediction.confidence}</span>
              </Row>
          ...
          <div className="mt-4 rounded-xl border border-dashed border-ledger-border-emphasis px-5 py-10 text-center">
            <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-ledger-elevated">
              <svg className="h-5 w-5 text-ledger-dimmed" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
              </svg>
            </div>
            <p className="mt-3 text-[13px] leading-relaxed text-ledger-muted">
      ...
      <div className="rounded-xl border border-ledger-border bg-ledger-elevated/50 p-5">
        <p className="text-[13px] leading-relaxed text-ledger-muted">
          <span className="font-medium text-ledger-text">Tip.</span> Small changes to rooms, area, or
...
function Row({ label, children, last = false }: { label: string; children: React.ReactNode; last?: boolean }) {
  return (
    <div className={`flex items-baseline justify-between py-2.5 ${last ? "" : "border-b border-ledger-border"}`}>
