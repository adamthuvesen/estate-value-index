...
- `source .venv/bin/activate` after `uv sync --all-extras`
- Run **pytest** before commit; for web changes also `cd web && npm test` (and lint/typecheck as needed)
...
**Train (typical):** `python train_model.py --use-features`  
**Pipeline (orchestrated):** e.g. `python -m src.estate_value_index.pipelines.core.complete_pipeline --quick` (see [agents/docs/data-pipeline.md](agents/docs/data-pipeline.md) and `complete_pipeline --help`)
