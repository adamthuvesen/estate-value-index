...
| `python -m estate_value_index.cli <cmd>` | Unified CLI dispatcher (`cli/__main__.py`): `crawl`, `batch`, `backfill`, `process`, `features`, `migrate`, `costs`, `areas`, `value-analysis`, `area-metrics` |
...
| Area | Where |
| ---- | ----- |
| Ingestion | `src/estate_value_index/ingestion/`, `scrapy.cfg`, `python -m estate_value_index.cli process` |
