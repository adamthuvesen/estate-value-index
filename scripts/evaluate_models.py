...
# Repo root (parent of scripts/) — make the src package importable when run directly.
SRC = Path(__file__).resolve().parents[1] / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from estate_value_index.ml import (  # noqa: E402  (import after sys.path setup)
...
def evaluate_models(
    ...
    for label, preds in predictions.items():
        for actual, predicted, listing_id in zip(y_test, preds, test_ids, strict=False):
